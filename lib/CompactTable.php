...
class CompactTable extends BasicTerminalObject {
	use StringLength;
	...
	private $prefix = "";
	/**
	 * @var false $header Force header row
	 */
	private $header;

	public function __construct(array $data, $prefix = "", $header = true) {
	...
	protected function getBorder() {
		return (new Border())->length($this->table_width)
							 ->result();
	}
